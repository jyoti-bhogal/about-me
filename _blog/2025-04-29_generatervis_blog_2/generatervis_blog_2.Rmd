---
title: "An R package to synthesise, visualise clinical data, and create a data workflow - Part 2"
subtitle: 
description: 
...
### ⬇️Using the package {generatervis}:
...
```{r, echo = TRUE}  
random_sample <- generatervis::rreads(patient_id = "patient_123", n = 2, read_length = 12)  
```

```{r, results='asis'}  
cat("```text\n")  
cat(paste(random_sample, collapse = "\n"))  
cat("\n```\n")  
```
